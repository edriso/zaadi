...
export function parsePreferences(raw) {
  try {
    ...
    if (!value || value.version !== 1)
      return { zoom: 1, city: '', method: '', hanafi: false };
    return {
...
// Only non-character shortcuts; native activation and scrolling remain available.
export function keyboardAction(
  ...
  if (
    ...
    return null;
  if (event.key === 'ArrowLeft') return 'next';
  if (event.key === 'ArrowRight') return 'previous';
